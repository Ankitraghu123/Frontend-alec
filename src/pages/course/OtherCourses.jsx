    ...
    try {
      ...
      if (response.data) {
        ...
        const filtered = response.data.filter(course => 
          course._id !== id && 
          course.subCategory?._id === response.data.find(c => c._id === id)?.subCategory?._id
        );
        setcourses(filtered);
      }
    } catch (error) {
              ...
              >
                <div className="d-flex flex-row" style={{ minHeight: '150px' }}>
                  ...
                  <Card.Body className="flex-grow-1">
                    <Card.Title className="h6">{course.Coursename || 'Untitled Course'}</Card.Title>
                    <Card.Text className="small">
                      <div>
                        <strong>Duration:</strong> {course.Durations || 'N/A'} months
                      </div>
                      <div>
